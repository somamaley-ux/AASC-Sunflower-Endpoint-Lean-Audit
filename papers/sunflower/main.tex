...
\begin{abstract}
This manuscript gives a kernel-forced AASC endpoint-transfer proof for the Erdős--Rado sunflower endpoint relative to fixed calibrated proof data \((\calC,\Complete^{\calC}_{k,H_k},H_k)\), where \(\calC\) is a declared-complete finite certificate language and \(H_k\ge H_k^{\calC}<\infty\) is a calibrated motif ceiling. The proof class is fixed-carrier endpoint transfer. The sunflower endpoint is represented by the core--petal residual matching carrier; a large no-sunflower countercase is routed through a calibrated finite certificate split; and the complete, above-ceiling, objective non-certified residual branch is excluded by the AASC kernel and the no-independent-discriminator consequence layer. The argument does not reproduce the random-restriction, spread-lemma, or entropy-compression routes in the current sunflower literature. Those are different proof classes and are not correctness standards for the present endpoint proof.
...
\paragraph{Proof at a glance.}
\[
\begin{array}{c}
\calF\subseteq\binom{U}{n}
\longrightarrow
(C\mapsto\calF_C)
\longrightarrow
\Sun_k(\calF)\equiv \exists C\,\nu(\calF_C)\ge k,\\[3pt]
\ExactCountercase+\NoSun_k+|\calF|>e^{H_k n}
\longrightarrow
\BMF^{\calC}_{k,H_k}(\calF)\ \vee\ \RBEsep^{\calC}_{k,H_k}(\calF),\\[3pt]
\BMF^{\calC}_{k,H_k}(\calF)\Rightarrow |\calF|\le e^{H_k n},\\[3pt]
\RBEsep^{\calC}_{k,H_k}(\calF)+\LocalEndpointUse
\Rightarrow \IndDisc\Rightarrow\bot.
\end{array}
\]
Thus the certified branch is entropy-bounded, while the calibrated residual branch is locally discharged by the kernel-forced no-independent-discriminator consequence.
...
\section{AASC Kernel and Consequence Layer Recap}

This subsection makes the kernel use in the sunflower proof self-contained.  The detailed kernel development is given in the AASC kernel and same-scope closure manuscripts \cite{MaleyKernel2026,MaleyClosure2026}; the local countercase pattern is used in the SAT and RH endpoint manuscripts \cite{MaleyPNP2026,MaleyRH2026}.  The present proof uses only the following consequence package.

\begin{center}
\fbox{\begin{minipage}{0.92\linewidth}
\textbf{Kernel roles.}
\begin{itemize}[leftmargin=2em]
\item \(\KRef\): fixes target, carrier, object identity, and lawful redescription.
\item \(\St\): supplies theorem-bearing or counterexample-bearing force and licensed reuse.
\item \(\Adm\): fixes the boundary between endpoint act, support, bookkeeping, selector import, carrier shift, and certified branch.
\item \(\Irr\): prevents same-act repair, deferred certification, and retrospective reclassification.
\end{itemize}
\[
\text{nondegenerate determinate same-carrier endpoint use}\Rightarrow \Kernel\Rightarrow\Aplus.
\]
\textbf{Consequence layer.} On a fixed carrier, \(\Aplus\) gives endpoint bivalence, AMetric no-selector discipline, unique admissible interior, no same-act repair, no carrier transfer, report preservation, and no independent same-domain endpoint-status discriminator.
\end{minipage}}
\end{center}

The no-independent-discriminator clause used below is not imported as a preference.  It is the fixed-domain form of the admissibility boundary: a status factor that changes endpoint standing on the same carrier is either bookkeeping, governance-equivalent to the defining endpoint interface, a carrier/domain shift, or forbidden independent status work.  This is the local consequence invoked in the residual-separator discharge.
...
\noindent\emph{Take-away.} A raw family is harmless as raw objecthood.  It becomes governed only when used as the exact countercase to the fixed core--petal endpoint.
...
\noindent\emph{Take-away.} The residual separator is evaluated under local endpoint-use discipline without being promoted to a global negative endpoint resolution.
...
\noindent\emph{Take-away.} Kernel denial does not preserve the same endpoint with fewer constraints; it exits or damages the determinate endpoint object.
...
\noindent\emph{Take-away.} The sunflower endpoint is exactly a role-cardinality occupation statement: one core supporting \(k\) disjoint residual petal roles.
...
\begin{definition}[Sunflower threshold]
For fixed \(k\), let \(f(n,k)\) be the least integer \(m\) such that every \(n\)-uniform family of size at least \(m\) contains a \(k\)-sunflower.  Equivalently,
\[
  f(n,k)-1=\sup\{|\calF|:\calF\subseteq\binom{U}{n}\text{ and }\NoSun_k(\calF)\}.
\]
\end{definition}
...
\[
  \Complete^{\calC}_{k,H_k}
\]
is a finite metacertificate for the proof instance: it records the declared certificate grammar \(\calC\), the calibrated ceiling \(H_k\), and the route-locus classification for bounded same-carrier negative support.  The completeness record does not require enumerating all conceivable sunflower-free families; it requires that every motif actually invoked as bounded negative support in the proof instance, or admitted by the route-locus classification for the families under consideration, has been assigned to one of the listed routes.  It means that the certificate language \(\calC\) has been fixed for the proof and every finite same-carrier negative structure used as endpoint-relevant support for a no-sunflower branch of entropy density at most \(H_k\) is classified in exactly one of the following ways:
\begin{enumerate}[label=(\roman*)]
...
\noindent\emph{Take-away.} The certified branch is a finite entropy certificate.  It is support for the bound, not a second endpoint authority.
...
\subsection{Quantitative framing}
Relative to fixed calibrated proof data \((\calC,\Complete^{\calC}_{k,H_k},H_k)\), the endpoint closeout gives a constant-base bound \(c_k^n\) for every \(c_k>e^{H_k}\).  This is the conjectural form of Erdős Problem \#20: for fixed \(k\), a constant-base bound is asymptotically stronger than the accepted \((C_k\log n)^n\)-type sunflower bounds in the current spread and entropy literature.  The present manuscript does not optimize \(H_k\).  The constant is controlled by the calibrated certificate ceiling rather than by a new spread-lemma estimate.

The calibration examples give lower constraints on \(H_k\), not an upper estimate.  Future certificate-language design asks which lawful negative motifs must be admitted and how large \(H_k^{\calC}\) must be once those motifs are included.  The gap of interest is the gap between the certified motif capacity and the true lawful no-sunflower capacity, not a defect of the kernel-forced endpoint closeout.

\subsection{Illustrative BMF certificate}
The product-transversal family has an explicit bounded motif factorization certificate.  Let the one-coordinate motif be
\[
  \calH_{k-1}=\bigl\{\{1\},\dots,\{k-1\}\bigr\},
\]
viewed as a rank-one sunflower-free motif.  Its \(n\)-fold tensor product is endpoint-isomorphic to \(\calT_{n,k}\).  The certificate records the raw sunflower-free motif, the product record \(\calH_{k-1}^{\boxtimes n}\), the identity injection, endpoint preservation, rank accounting \(\sum_i1=n\), and entropy accounting
\[
  |\calT_{n,k}|=(k-1)^n=e^{n\log(k-1)}.
\]
Thus \(\calT_{n,k}\) belongs to the certified branch at any ceiling \(H_k\ge\log(k-1)\).  It is not a residual separator.
...
For the fixed core--petal carrier, local endpoint use instantiates the kernel by Theorem~\ref{thm:adequacy-kernel}.  Reference fixes the carrier and endpoint image; standing supplies local countercase force; admissibility fixes the boundary between certified support, bookkeeping, carrier shift, and endpoint-status work; and irreversibility prevents same-act repair.  On that fixed carrier, the \(\Aplus\) layer gives a unique admissible interior, AMetric no-selector discipline, and no independent same-domain endpoint-status discriminator.  The residual separator is tested against this local consequence, not against a preference for any external proof method.
...
\noindent\emph{Take-away.} Thus the calibrated residual separator cannot survive as a determinate local countercase on the fixed core--petal carrier.
...
\noindent\emph{Take-away.} The residual countercase is discharged locally.  The certified branch is the only endpoint-admissible negative route above the calibrated ceiling relative to the fixed proof data.
...
\paragraph{Toward AASC-free certificate extraction.}
The parallel reconstruction route would give an effective or finite proof that any \(\NoSun_k\) family in the target class admits a \(\BMF^{\calC}_{k,H_k}\) certificate, or that a stated finite obstruction forces \(\Sun_k\).  Such a theorem would have to build the injection record, endpoint-preservation record, rank accounting, and entropy accounting directly.  Natural questions are whether maximal sunflower-free motifs below a given entropy density can be recognized by a controlled extension of \(\calC\), and whether the resulting \(H_k^{\calC}\) remains finite under the motifs actually used by large sunflower-free families.
...
Two natural lines of continuation remain.  Inside the AASC endpoint-transfer program, the next task is to enrich certificate languages \(\calC\) while keeping the calibrated ceiling \(H_k^{\calC}\) explicit and finite.  This means identifying which product, design, geometric, algebraic, or other finite motif records should be admitted as lawful negative support and how their entropy densities affect the calibrated ceiling.

A separate AASC-free reconstruction route would attempt to replace the kernel-forced discharge of the residual separator by finite certificate extraction or direct combinatorial exclusion.  That route would prove, without using endpoint-discriminator closure, that every no-sunflower family in the target class either has a \(\BMF^{\calC}_{k,H_k}\) certificate or contains a core with \(k\) disjoint residual petals.  It is a valuable parallel research front, not a missing premise of the present proof class.
...
\addlinespace
The completeness record is doing too much work. & The completeness record is fixed proof-instance data: a finite route-locus metacertificate classifying bounded negative-support routes before residual status is available.  If a bounded route is unencoded, \(\calC\) must be extended; it does not become residual separator work. \\
\bottomrule
\end{tabular}
\end{center}
